...
class TranslateController extends BaseController{
	...
	private function add()
	{
		...
		if(isset($_POST['key'], $_POST['value'], $_POST['comment']))
		{
            ...
            $insert_id = $this->db->insert_id("INSERT INTO `".$this->tb."` SET `key`=?, comment=?", $param);
			...
			if(isset($_POST['save_id'])&&is_array($_POST['save_id']))
			{
				if(isset($_POST['save_id'], $_POST['key'], $_POST['value'], $_POST['comment']))
				{
					for($i=0; $i<=count($_POST['save_id']) - 1; $i++)
					{
                        ...
                        $param = array($_POST['key'][$i], $_POST['comment'][$i], $_POST['save_id'][$i]);
                        $this->db->query("UPDATE `".$this->tb."` SET `key`=?, `comment`=? WHERE id=?", $param);
					}
