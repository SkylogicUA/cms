...
class PagesController extends BaseController{
	...
	private function duplicate($row)
	{
		...
		{
			$message='';
			if(isset($row['active'], $row['name'])&&$row['name']!="")
			{
				$name=$row['name'].' [copy]';
				$url = translit($name);
				//echo $url.'asd';
				$param = array($row['active'], $row['form']);
				$insert_id = $this->db->insert_id("INSERT INTO `".$this->tb."` SET `active`=?, `form`=?", $param);
				$message = $this->checkUrl($this->tb, $url, $insert_id);
				foreach($this->language as $lang)
				{
					$tb=$lang['language']."_".$this->tb;
					$param = array($name, $row['title'], $row['keywords'], $row['description'], $row['body'], $insert_id);
					$this->db->query("INSERT INTO `$tb` SET `name`=?, `title`=?, `keywords`=?, `description`=?, `body`=?, `".$this->tb."_id`=?", $param);
				}
				header("Location: /admin/{$this->tb}/edit/".$insert_id);
		...
		else
		{
			if(isset($_POST['save_id'])&&is_array($_POST['save_id']))
			{
				...
				if(isset($_POST['active'], $_POST['url'], $_POST['id'], $_POST['name'], $_POST['title'], $_POST['keywords'], $_POST['description'], $_POST['body']))
				{
					...
					$this->db->query("UPDATE `".$this->tb."` SET `active`=?, `form`=? WHERE id=?", $param);
