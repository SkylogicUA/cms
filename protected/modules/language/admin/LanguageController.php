	...
	private function delete(){
		...
		if(isset($this->registry['access']))$message = $this->registry['access'];
			 ...
			 if(isset($this->params['delete'])&& $this->params['delete']>0)  {
				...
				$my_value = $key; 
				$array=$tables;
				$filtered_array = array_filter($array, function ($element) use ($my_value) { $mm =explode('_',$element["Tables_in_football"]);	 return ($mm[0] == $my_value); } ); 
				
				 
				foreach($filtered_array as $ky=>$val){
					 $this->db->query("DROP TABLE `".$val["Tables_in_football"]."`  " );
